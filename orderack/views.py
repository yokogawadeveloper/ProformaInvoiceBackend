...
##        data['Freight_Percent'] = proforma_serializer.data['items'][0]['FreightPercent']
    ...
    def generatePDFWithSBI(self, request, id):
        ...
        data['DocNo'] = str(proforma_serializer.data['DocNo'])
        if proforma_serializer.data['DocDate'] is not None:
            data['DocDate'] = datetime.datetime.strptime(str(proforma_serializer.data['DocDate']),
                                                         '%Y-%m-%d').date()
            data['DocDate'] = data['DocDate'].strftime("%d/%m/%Y")
        else:
            data['DocDate'] = ""
        ...
        data['region'] = regionMaster.objects.filter(RegionId=data['RegionId']).values('RegionName')[0][
            'RegionName']
    ...
    def download_excel(self, request):
        ...
        response = HttpResponse(FilePointer.read(),content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
